...
class App extends Component {
  ...
  activeVisit = id => {
    ...
    const filterList = countriesList.map(obj => {
      if (obj.id === id) {
  ...
  render() {
    ...
    const watchList = countriesList.filter(each => each.isVisited === true)

    return (
      <div className="main-container">
        <h1 className="heading1">Countries</h1>
        <ul className="visitList-container">
          {countriesList.map(each => (
            <li key={each.id}>
              <div className="item-container">
                <p className="name-para">{each.name}</p>
                {each.isVisited ? (
                  <p className="visit-active">Visited</p>
                ) : (
                  <button
                    type="button"
                    onClick={() => {
                      this.activeVisit(each.id)
                    }}
                    className="visit-plain"
                  >
                    Visit
                  </button>
                )}
                ...
                <li key={each.id}>
                  <div className="list-item-style">
                    ...
                    <div className="bottom-container">
                      <p className="country-para">{each.name}</p>
                      <button
                        type="button"
                        className="btn-style"
                        onClick={() => {
                          this.removeItem(each.id)
                        }}
                      >
                        Remove
                      </button>
